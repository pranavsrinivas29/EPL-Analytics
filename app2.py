...
# Load the FAISS index and the combined data
index = faiss.read_index('faiss_index.idx')
combined_data = pd.read_csv('combined_data_with_embeddings.csv')
...
def get_relevant_context(question, index, data, model, k=1):
    question_embedding = model.encode([question])
    distances, indices = index.search(question_embedding, k)
    context = " ".join(data.iloc[indices[0]]['text'])
    return context

# Function to generate a response using LLM and RAG
def generate_answer(question, context, generator):
    input_text = f"Question: {question}\nContext: {context}\nAnswer:"
    answer = generator(input_text, max_length=50, num_return_sequences=1)[0]['generated_text']
...
if page == "Forecasting":
    ...
    user_question = st.text_input("Ask a question about the Premier League:")

    if user_question:
        # Get the relevant context from the combined data
        context = get_relevant_context(user_question, index, combined_data, embedding_model)
        
        # Generate an answer using the LLM and context
        response = generate_answer(user_question, context, generator)
